...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while index < value.len() {
            let len = usize::from_be_bytes(value[index..index + Chunk::LENGTH_SIZE].try_into()?);
            println!("len done");
            let chunk_type_bytes: [u8; 4] = value
                    [index + Chunk::LENGTH_SIZE..index + Chunk::TYPE_SIZE]
                .try_into()?;
            println!("chunk_type_bytes done");
            let chunk_type: ChunkType = ChunkType::try_from(chunk_type_bytes)?;
            println!("chunk_type done");
            let bytes: Vec<u8> = value[index..index + len + Chunk::TYPE_SIZE].to_vec();
            println!("bytes done");
            index += len + Chunk::TYPE_SIZE;

            chunks.push(Chunk::new(chunk_type, bytes));
        }
